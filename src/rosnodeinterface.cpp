...
// C/C++ source code generated on : Wed Nov 01 14:56:33 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"CC_Example\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(CC_Example_M, (NULL));
        CC_Example_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(CC_Example_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(CC_Example_M)
                == (NULL)) || rtmGetStopRequested(CC_Example_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(CC_Example_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
